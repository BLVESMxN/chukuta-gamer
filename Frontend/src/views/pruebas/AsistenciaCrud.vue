<template>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="asistencia in asistencias" :key="asistencia.id">
              ...
              <td>
                <select v-model="asistencia.estado" @change="actualizarEstado(asistencia)">
                  <option value="Presente">Presente</option>
                  <option value="Ausente">Ausente</option>
                  <option value="Permiso">Permiso</option>
                </select>
  ...
  <script>
  import { AsistenciasEstudiantesService } from "@/modelo/AsistenciasEstudiantes.mjs"; // Importamos el servicio de asistencias
  ...
  export default {
    ...
    methods: {
      ...
      async agregarAsistencia() {
        try {
          const { estudianteId, asignaturaId, fecha, estado } = this.nuevaAsistencia;
  
          await this.asistenciasService.agregarAsistencia(estudianteId, asignaturaId, fecha, estado);
      ...
      async actualizarEstado(asistencia) {
        try {
          await this.asistenciasService.actualizarAsistencia(asistencia.id, asistencia.estado);
