<template>
    <div class="listado-tareas">
      ...
      <table class="tabla-tareas">
        ...
        <tbody>
          <tr v-for="(tarea, index) in tareas" :key="index">
            ...
            <td>{{ tarea.calificacion !== null ? tarea.calificacion : 'N/A' }}</td>
            <td :class="{ entregado: tarea.entregado, noEntregado: !tarea.entregado }">
      ...
      <p class="texto-progreso">Progreso de tareas entregadas: {{ porcentajeEntregado }}%</p>
    </div>
  </template>
  ...
  export default {
    ...
    computed: {
      porcentajeEntregado() {
        ...
        const tareasEntregadas = this.tareas.filter(tarea => tarea.entregado).length;
