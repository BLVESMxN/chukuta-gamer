<template>
      ...
      <div id="app">
        ...
        <div class="form-container">
          ...
          <div class="form-field">
            ...
            <select v-model="nuevoPeriodo.trimestre" class="input-field">
              <option value="1">1º Trimestre</option>
              <option value="2">2º Trimestre</option>
              <option value="3">3º Trimestre</option>
            </select>
          ...
          <div class="form-field">
            ...
            <input type="date" v-model="nuevoPeriodo.fecha_fin" class="input-field" />
          </div>
  
          <button @click="agregarPeriodo" class="add-button">Agregar Periodo</button>
        </div>
  
        <!-- Formulario para editar periodo -->
        <div v-if="periodoEditado" class="form-container">
          <h2>Editar Periodo</h2>
  
          <div class="form-field">
            <label for="anio">Año</label>
            <select v-model="periodoEditado.anio" class="input-field">
              <option v-for="anio in aniosDisponibles" :key="anio" :value="anio">{{ anio }}</option>
            </select>
          </div>
          ...
          <div class="form-field">
            <label for="trimestre">Trimestre</label>
            <select v-model="periodoEditado.trimestre" class="input-field">
              <option value="1">1º Trimestre</option>
              <option value="2">2º Trimestre</option>
              <option value="3">3º Trimestre</option>
            </select>
          ...
          <div class="form-field">
            <label for="fecha_inicio">Fecha de Inicio</label>
            <input type="date" v-model="periodoEditado.fecha_inicio" class="input-field" />
          </div>
  
          <div class="form-field">
            <label for="fecha_fin">Fecha de Fin</label>
            <input type="date" v-model="periodoEditado.fecha_fin" class="input-field" />
          </div>
  
          <button @click="editarPeriodo" class="save-button">Guardar Cambios</button>
  ...
  export default {
    ...
    computed: {
      ...
      periodosOrdenados() {
        return [...this.periodosFiltrados].sort((a, b) => {
          if (this.ordenAnioAscendente) {
            return a.anio - b.anio;
          } else {
            return b.anio - a.anio;
          }
        }).sort((a, b) => {
          if (this.ordenTrimestreAscendente) {
      ...
      // Años disponibles desde 2024 hasta 2034
      aniosDisponibles() {
