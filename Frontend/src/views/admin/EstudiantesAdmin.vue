    ...
    <div>
      ...
      <div class="filter-container">
        ...
        <div class="form-field">
          ...
          <select v-model="filtroColegio" @change="filtrarEstudiantesPorColegio" class="input-field">
      ...
      <div class="form-container">
        ...
        <div class="form-field">
          ...
          <input v-model="estudiante.name" placeholder="Nombre del Estudiante" class="input-field" />
        </div>
  
        <div class="form-field">
          <label for="colegio">Colegio</label>
          <select v-model="estudiante.colegio" @change="cargarGradosPadres" class="input-field">
            <option v-for="colegio in colegios" :key="colegio.id" :value="colegio.id">{{ colegio.nombre }}</option>
          </select>
        </div>
        ...
        <div class="form-field">
          ...
          <select v-model="estudiante.grado" class="input-field">
            <option v-for="grado in grados" :key="grado.id" :value="grado.id">{{ mostrarGrado(grado) }}</option>
          </select>
        </div>
  
        <div class="form-field">
          <label for="user_padre">Correo del Padre</label>
          <input v-model="estudiante.email_padre" placeholder="Correo del Padre" class="input-field" />
        </div>
  
        <div class="form-field">
          <label for="user_madre">Correo de la Madre</label>
          <input v-model="estudiante.email_madre" placeholder="Correo de la Madre" class="input-field" />
        </div>
  
        <button @click="editando ? actualizarEstudiante() : agregarEstudiante()" class="add-button">
          ...
          <tbody>
            <tr v-for="estudiante in estudiantesFiltrados" :key="estudiante.id">
              ...
              <td>
                ...
                <button @click="eliminarEstudiante(estudiante.id)" class="delete-button">Eliminar</button>
              </td>
            </tr>
          </tbody>
        </table>
    ...
    computed: {
      estudiantesFiltrados() {
        ...
        if (this.filtroColegio) {
          estudiantesFiltrados = estudiantesFiltrados.filter(estudiante => estudiante.colegio === parseInt(this.filtroColegio));
        }
  
        // Filtrar por nombre
        if (this.busquedaNombre) {
          estudiantesFiltrados = estudiantesFiltrados.filter(estudiante => 
            estudiante.name.toLowerCase().includes(this.busquedaNombre.toLowerCase())
          );
        }
    ...
    methods: {
      ...
      async cargarGradosPadres() {
        if (this.estudiante.colegio) {
          this.grados = await this.modeloEstudiantes.obtenerGradosPorColegio(this.estudiante.colegio);
        }
      },
      ...
      async agregarEstudiante() {
        try {
          // Verificar que los correos pertenezcan al colegio
          const padreValido = await this.modeloEstudiantes.verificarCorreoPadre(this.estudiante.colegio, this.estudiante.email_padre);
          const madreValida = await this.modeloEstudiantes.verificarCorreoPadre(this.estudiante.colegio, this.estudiante.email_madre);
  
          // Verificar que el grado pertenezca al colegio seleccionado
          const gradoValido = await this.modeloEstudiantes.verificarGradoPorColegio(this.estudiante.colegio, this.estudiante.grado);
  
          if (!padreValido || !madreValida) {
            this.errorMensaje = "Uno o ambos correos no corresponden a los padres registrados en este colegio.";
            return;
          }
  
          if (!gradoValido) {
            this.errorMensaje = "El grado seleccionado no corresponde al colegio seleccionado.";
            return;
          }
      ...
      async actualizarEstudiante() {
        try {
          ...
          await this.modeloEstudiantes.actualizarEstudiante(this.estudianteId, this.estudiante);
          await this.cargarDatos();
          this.limpiarFormulario();
          this.editando = false;
        } catch (error) {
